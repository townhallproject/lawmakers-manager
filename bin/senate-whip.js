...
const getMetaData = (callback) => {
    return googleMethods
        .read(sheets, SHEETS_ID, "Whip Count Metadata!A2:T")
        .then((googleRows) => {
            ...
            return rows.forEach(async (row, index) => {
                ...
                if (
                    googleMethods.convertColumnToNumber(citationColumn) >
                    googleMethods.convertColumnToNumber(endColumn)
                ) {
                    endColumn = citationColumn;
                }
                topics.push(dataToWrite);
                return firebasedb.firestore
    ...
    googleMethods
        ...
        .then((googleRows) => {
            ...
            googleRows.forEach(async (row, index) => {
                const [memberId, firstName, lastName, party] = row;
                ...
                const issues = topics.reduce((acc, topic) => {
                    const statusIndex =
                        googleMethods.convertColumnToNumber(topic.statusColumn) - 1;
                    const citationIndex =
                        googleMethods.convertColumnToNumber(topic.citationColumn) - 1;
                ...
                const snapshot = await firebasedb.firestore
                    .collection("office_people")
                    .doc(memberId)
                    .get();
                ...
                const senator = new Senator(data, issues);
